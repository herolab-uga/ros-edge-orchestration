...
def simple_move(x,y,w,z):
    ...
    rospy.loginfo("Connected to R0 move base server")
    rospy.loginfo("Starting R0 navigation task")
    #send goal
    ...
    if not finished_within_time:
        ...
        rospy.loginfo("R0 Timed out achieving goal")
    else:
        # We made it!
        state = move_base.get_state()
        print("State:", state)
        if state == 3:
            rospy.loginfo("R0 Goal succeeded!")
...
if __name__ == '__main__':
    pub = rospy.Publisher('R0/task_status',TaskStatus, queue_size=10)
    ...
    try:
        start = timer()
        rospy.init_node('simple_move_tb3_0')
        rate = rospy.Rate(10)

        arr = [  0.0, 0.0, 0.0, 1.0,
                -3.9018571376800537, 5.906538009643555, 0.0, 1.0,
                -4.799447059631348, 6.3402113914489755, 0.0, 1.0,
                -3.9871771335601807, 0.2986183762550354, 0.0, 1.0,
                -3.897684574127197, -2.453843593597412, 0.0, 1.0,
                -0.705956220626831, -2.223165273666382, 0.0, 1.0,
                3.1693997383117676, 1.8752059936523438, 0.0, 1.0,
                3.6914889812469482, -4.054971694946289, 0.0, 1.0,
                6.025975227355957, -1.615718126296997, 0.0, 1.0,
                5.701817512512207, 0.5146491527557373, 0.0, 1.0,
                5.594733238220215, 1.5674710273742676, 0.0, 1.0,
        ...
        for i in range(10):
            print("-------------------------------------------")
            print("-------------------------------------------") 
            print("Sending Goal {} to R0".format(i))
            goal = coordinates[i]

...
class PosePublisher():
    ...
    def publish(self):
        self.obj_pub = rospy.Publisher("/tb3_0/Objects_found", ObjectFound, queue_size=10)
        self.obj_pose_pub = rospy.Publisher("tb3_0/Objects_poses", ObjectPose, queue_size=10)
        self.pub_rate = rospy.Rate(10)

    def subscribe(self):
        self.bbox_sub = rospy.Subscriber('/tb3_0/yolov5/detections', BoundingBoxes, self.bbox_callback)
        self.pc_sub = rospy.Subscriber('/tb3_0/camera/depth/points', PointCloud2, self.pc_callback)
        self.info_sub = rospy.Subscriber('/tb3_0/camera/depth/camera_info', CameraInfo, self.info_callback)
    ...
    def obj_coord(self, centroid):
        ...
        point_pub = rospy.Publisher('/tb3_0/target_point', PointStamped, queue_size=1)
        pc2_point_msg = PointStamped()
        pc2_point_msg.header.frame_id = "/tb3_0/camera_rgb_optical_frame"
        pc2_point_msg.header.stamp = rospy.Time(0)
        pc2_point_msg.point = self.pc2_point

        # get transform
        self.tf_listener.waitForTransform('/tb3_0/camera_rgb_optical_frame', '/map', rospy.Time(), rospy.Duration(4))
        ...
        if dist <= self.dist_thresh:
            if self.box_class in self.objects:
                ...
                print("Objects collected by R3: {}".format(self.poses))
                print("----------------------------------------------")
...
def main():
    rospy.init_node("R3_pose_publisher")
